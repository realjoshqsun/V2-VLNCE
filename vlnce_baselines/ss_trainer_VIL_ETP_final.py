...
@baseline_registry.register_trainer(name="VIL-ETP-Trainer-final")
class VIL_ETPTrainer(ETPTrainer):
    ...
    def save_checkpoint(self, iteration: int, save_optim=True):
        if save_optim:
            torch.save(
                obj={
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            def prepare_config_envs(index=0):
                ...
                if self.config.IL.view_sampling == 'height_only':
                    ...
                    height_distribution = getattr(self.config.IL, 'height_distribution', 'sqrt_uniform')
                    assert height_distribution in ['sqrt_uniform', 'uniform']
                    if height_distribution == 'sqrt_uniform':
                        u = np.sqrt(np.random.uniform(0, 1))
                    elif height_distribution == 'uniform':
                    ...
                    height_shift = np.random.uniform(self.config.IL.min_height_shift, self.config.IL.max_height_shift)
                    angle_shift = np.random.uniform(self.config.IL.min_angle_shift, self.config.IL.max_angle_shift)
                    euler_angle = angle_shift * np.pi / 180
            ...
            try:
                ...
                logs = self._train_interval(interval, self.config.IL.ml_weight, self.config.IL.cl_weight, self.config.IL.wpd_weight, sample_ratio)
                ...
                if self.local_rank < 1:
                    ...
                    if cur_iter > 8000: # TODO: This should be deleted in the final version. This is to save space for useful ckpts and for faster evaluation.
                        self.save_checkpoint(cur_iter, save_optim=False)
            except Exception as e:
                logger.error(f"Error at iteration {cur_iter}: {e}")
                self.save_checkpoint(cur_iter, save_optim=True) 
                break 

    def _train_interval(self, interval, ml_weight, cl_weight, wpd_weight, sample_ratio):
        if self.local_rank < 1:
        ...
        for idx in pbar:
            ...
            with autocast():
                self.rollout('train', ml_weight, cl_weight, wpd_weight, sample_ratio)
    ...
    def rollout(self, mode, ml_weight=None, cl_weight=None, wpd_weight=None, sample_ratio=None):
        if mode == 'train':
            feedback = 'sample'
        ...
        episode_ids = [episode.episode_id for episode in self.envs.current_episodes()]
        # print('in rollout, self.envs.current_episodes()', os.environ['LOCAL_RANK'], episode_ids)
