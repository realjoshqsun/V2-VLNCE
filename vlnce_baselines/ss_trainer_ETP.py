...
class ETPTrainer(BaseVLNCETrainer):
    ...
    def _initialize_policy(
        ...
        if load_from_ckpt:
            ...
            # reset_start_iter = getattr(config.IL, "fine_tune", False)
            # if (not eval) and reset_start_iter:
            #     start_iter = 0
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            try:
                ...
                if self.local_rank < 1:
                    ...
                    self.save_checkpoint(cur_iter, save_optim=(cur_iter==total_iter))
            except Exception as e:
                logger.error(f"Error at iteration {cur_iter}: {e}")
                self.save_checkpoint(cur_iter, save_optimizer=True) 
                break 
