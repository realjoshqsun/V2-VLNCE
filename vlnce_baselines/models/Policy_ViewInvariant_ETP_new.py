...
class PolicyViewInvariantETP_new(ILPolicy):
    def __init__(
        self,
...
class VILETP(Net):
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
        ...
        elif mode == 'waypoint_contrastive':
            batch_size = observations['rgb'].shape[0]
            ...
            if np.random.rand() > self.keep_origin_view_wp_prob:
                ...
                waypoint_heatmap_logits = self.waypoint_predictor_teacher(
                    rgb_embedding, depth_embedding)
