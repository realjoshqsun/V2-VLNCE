...
class PolicyVIL_ETP(ILPolicy):
    def __init__(
        ...
        def count_parameters(model):
            ...
            percent = 100.0 * trainable / total if total > 0 else 0.0
            return total, trainable, percent
        total, trainable, percent = count_parameters(self.net)
        print(f"[PolicyVIL_ETP] Total params: {total/1e6:.2f} M")
        print(f"[PolicyVIL_ETP] Trainable params: {trainable/1e6:.2f} M")
        print(f"[PolicyVIL_ETP] Trainable ratio: {percent:.2f}%")
...
class VILETP(Net):
    def __init__(
        ...
        print('\nLoading Waypoint Predictor to ETP model ...')
        from vlnce_baselines.waypoint_pred.TRM_net import BinaryDistPredictor_TRM
    ...
    def forward(self, mode=None, 
                ...
                observations=None, in_train=True,
        ...
        if mode == 'language':
            ...
            def forward_obs(obs, suffix=''):
                ...
                rgb_embedding = self.rgb_encoder(obs_view12)      # torch.Size([bs, 512])
            ...
            rgb_embedding, depth_embedding = forward_obs(observations)
            rgb_embedding_aug, depth_embedding_aug = forward_obs(observations, suffix='_aug')
